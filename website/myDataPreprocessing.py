...
# Label Encoder for converting categorical values to numerical values
    ...
    for i in range(k):
        name=obj_features
        name[i]
        df[name[i]]= label_encoder.fit_transform(df[name[i]].astype(str))
        joblib.dump(label_encoder, f'../model/{name[i]}_encoder.save')
        print(f"Feature '{name[i]}' encoded")
...
# Feature scaling - Normalization
    ...
    joblib.dump(minmax_scaler, '../model/minmax_scaler.save')
